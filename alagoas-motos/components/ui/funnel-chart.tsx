...
function SegmentLabel({
  ...
  const valueEl = showValues && (
    <span className="whitespace-nowrap font-semibold text-[var(--text-primary)] text-sm">
      {display}
    </span>
  );
  const pctEl = showPercentage && (
    <span className="rounded-full bg-[var(--text-primary)] px-3 py-1 font-bold text-[var(--bg-panel)] text-xs shadow-sm">
      {formatPercentage(pct)}
    </span>
  );
  const labelEl = showLabels && (
    <span className="whitespace-nowrap font-medium text-[var(--text-muted)] text-xs">
      {stage.label}
    </span>
  );
...
export function FunnelChart({
  ...
  const max = Math.max(first.value, ...data.map((stage) => stage.value), 1);
  const n = data.length;
  const norms = data.map((stage) => Math.max(0, stage.value) / max);
          ...
          {data.map((stage, i) => {
            ...
            return (
              <motion.div
                ...
                aria-label={onStageClick ? `Filtrar leads da etapa ${stage.label}` : undefined}
                key={`lbl-${stage.label}`}
